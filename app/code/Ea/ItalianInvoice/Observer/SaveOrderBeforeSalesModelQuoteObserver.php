...
class SaveOrderBeforeSalesModelQuoteObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $billingExtensionAttributes = $quote->getBillingAddress()->getExtensionAttributes();

        ///////SAVE fiscal_sdi to BILLING
        $sdi = $quote->getBillingAddress()->getFiscalSdi();
        if (is_null($sdi)){
            if (!is_null($billingExtensionAttributes)) {
                $sdi_ext = $billingExtensionAttributes->getFiscalSdi();
        ...
        if (is_null($cf)){
            if (!is_null($billingExtensionAttributes)) {
                $cf_ext = $billingExtensionAttributes->getFiscalCode();
        ...
        if (is_null($company)){
            if (!is_null($billingExtensionAttributes)) {
                $company_ext = $billingExtensionAttributes->getFiscalCompany();
