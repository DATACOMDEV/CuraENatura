...
abstract class Checkout extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    public function getLineItemsForOrder($order, $subscriptions)
    {
		...
		//DATACOM niente calcoli, si fa ritornare una riga con l'importo totale dell'ordine

		$lines[] = [
			'price_data' => [
				'currency' => $currency,
				'product_data' => [
					'name' => 'Pagamento ordine #'.$order->getIncrementId(),
				...
				'unit_amount' => $this->helper->convertMagentoAmountToStripeAmount($order->getGrandTotal(), $currency)
			],
			'quantity' => 1
		];
        ...
        foreach ($orderItems as $orderItem)
        {
            ...
            $taxRate = $this->subscriptions->retrieveTaxRate($orderItem->getTaxPercent(), $this->config->priceIncludesTax());
    ...
    protected function getSessionParams($order)
    {
		...
		//DATACOM controllo sconto disattivato
