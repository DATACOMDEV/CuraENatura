...
class gateway {
    ...
    public function exportOrder($status, $orderId) {
        try {
            ...
            $query = "SELECT o.entity_id, o.status, o.increment_id, a.firstname customer_firstname, a.lastname customer_lastname, ADDTIME(o.created_at, '0 2:00:00.00') AS created_at, o.shipping_invoiced, o.shipping_tax_amount, o.msp_cod_amount, o.msp_cod_tax_amount, i.tax_percent, i.row_invoiced, i.tax_invoiced FROM #prefix#sales_order o INNER JOIN #prefix#sales_order_item i ON o.entity_id=i.order_id INNER JOIN #prefix#sales_order_address a ON a.parent_id=i.order_id AND a.address_type='billing' WHERE o.status=? AND o.entity_id>=? ORDER BY o.created_at DESC";
            ...
            $strCSV .= "STATO;N. ORDINE;DATA;CLIENTE;NETTO IVA 22%;ALIQUOTA IVA 22%;NETTO IVA 10%;ALIQUOTA IVA 10%;NETTO IVA 4%;ALIQUOTA IVA 4%;SPEDIZIONE;CONTRASSEGNO;NOTE\r\n";
            foreach ($orders as $order) {
                ...
                if ($id != $oldId) {
                    if (!$blnFirstRow) $strCSV .= sprintf("%s;%s;%s;%s;%s;%s;%s;%s;%s\r\n", $netto22, $iva22, $netto10, $iva10, $netto4, $iva4, $sped, $cod, $msg);
                ...
                if ($id == $oldId) {
                    ...
                    switch ($order->tax_percent) {
                        case 22:
                            $netto22 += $order->row_invoiced;
                            $iva22 += $order->tax_invoiced;
                            break;
                        case 10:
                            $netto10 += $order->row_invoiced;
                            $iva10 += $order->tax_invoiced;
                            break;
                        case 4:
                            $netto4 += $order->row_invoiced;
