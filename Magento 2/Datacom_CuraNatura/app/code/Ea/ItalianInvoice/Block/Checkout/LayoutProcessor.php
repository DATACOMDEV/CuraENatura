...
class LayoutProcessor implements \Magento\Checkout\Block\Checkout\LayoutProcessorInterface
{
    ...
    public function getField($attributeCode, $scope)
    {
		...
		if ($attributeCode != 'fiscal_company') {
			$field['component'] = 'Magento_Ui/js/form/element/abstract';
			$field['validation'] = [
                'validate-'.$attributeCode => 1
            ];
            
			if ($attributeCode == 'fiscal_code') {
                $field['label'] = __('Fiscal Code');
			}
		}
