...
class gateway {
    ...
    public function exportOrder($status, $orderId) {
        try {
            ...
            $query = "SELECT o.entity_id, o.status, o.state, o.increment_id, a.firstname customer_firstname, a.lastname customer_lastname, ADDTIME(o.created_at, '0 2:00:00.00') AS created_at, o.shipping_invoiced, o.shipping_tax_amount, o.msp_cod_amount, o.msp_cod_tax_amount, i.tax_percent, i.row_invoiced, i.tax_invoiced, o.applied_rule_ids, o.base_discount_amount, i.base_discount_amount order_item_discount, i.discount_tax_compensation_amount, i.base_row_total, i.base_tax_amount FROM #prefix#sales_order o INNER JOIN #prefix#sales_order_item i ON o.entity_id=i.order_id INNER JOIN #prefix#sales_order_address a ON a.parent_id=i.order_id AND a.address_type='billing' WHERE o.status=? AND o.entity_id>=? ORDER BY o.created_at DESC";
            ...
            foreach ($orders as $order) {
                ...
                if ($id != $oldId) {
                    ...
                    $isOrderInvoiced = $order->state == 'complete';
                    //Primo id ordine con nuova gestione spedizione gratuita: 6342
                    $isNewFreeShipping = $id >= 6342;

                    $oldId = $id;
                    // $netto22 += $order->shipping_invoiced;
                    // $iva22 += $order->shipping_tax_amount;
                    $sped = $order->shipping_invoiced;
                    if (empty($sped)) {
                        $sped = 0;
                    }
                    $sped = $sped + $order->shipping_tax_amount;
                    ...
                    $sconto = $order->base_discount_amount;
                    if (empty($sconto)) {
                        $sconto = 0;
                    }
                    $sconto = abs($sconto);
                    if ($isNewFreeShipping) {
                ...
                if ($id == $oldId) {
                    ...
                    $rowNetto = $order->row_invoiced;
                    $rowAliquota = $order->tax_invoiced;
                    if (!$isOrderInvoiced) {
                        $rowNetto = $order->base_row_total;
                        $rowAliquota = $order->base_tax_amount;
                    }
                    switch ($order->tax_percent) {
                        case 22:
                            $netto22 += $rowNetto + $orderItemTaxCompensation - $orderItemDiscount;
                            $iva22 += $rowAliquota;
                            break;
                        case 10:
                            $netto10 += $rowNetto + $orderItemTaxCompensation - $orderItemDiscount;
                            $iva10 += $rowAliquota;
                            break;
                        case 4:
                            $netto4 += $rowNetto + $orderItemTaxCompensation - $orderItemDiscount;
                            $iva4 += $rowAliquota;
                            break;
                        default:
